...
#########################################################################################################################
            if request[0] == constChord.LOOKUP_REQ:  # A lookup request
                ...
                print("Sender: {}, Actual Chord: {}, Next Chord: {}, Key: {}".format(sender, self.node_id, next_id, request[1]))
                

                if (self.node_id == next_id):
                    print()
                    time.sleep(0.2)
                    print("found key, sending back\n")

                    # send current id back to sender
                    self.channel.send_to([sender], (constChord.LOOKUP_REP, sender, self.node_id))
                else:
                    # wrong node, so go on
                    print()
                    print("ID for key not found yet, get next ID: " + str(next_id)) 
                    print()
                    
                    # forward for another round in the lopp, cause key is not found yet
                    self.channel.send_to([str(next_id)], (constChord.LOOKUP_REQ, request[1], [self.node_id]))
                    
                    # wait for answer from next node (not needed fopr working, but the )
                    result = self.channel.receive_from([str(next_id)])
                    time.sleep(1)
                    
                    # send answer from next node, back to sender
                    self.channel.send_to([str(sender)], (constChord.LOOKUP_REQ, self.node_id, result))
#########################################################################################################################
