...
class DummyChordClient:
    ...
    def run(self):
        nodeList = self.channel.channel.smembers('node')
        key = str(int(random.choice(tuple(nodeList))))
        randomChord = str(int(random.choice(tuple(nodeList))))
        clientID = self.node_id
        
        print("\nNode List: {} \nClientID: {} \nRandon Key: {} \nRandom Chord: {}\n"
              .format(nodeList, clientID, key, randomChord))
        

        self.channel.send_to([randomChord], (constChord.LOOKUP_REQ, key, clientID))
        result = self.channel.receive_from([randomChord])
